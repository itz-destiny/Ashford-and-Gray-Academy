  ...
  for (const email of EMAILS) {
    try {
      ...
      try {
        ...
        await admin.auth().updateUser(fbUser.uid, { password: tempPwd, emailVerified: true });
      ...
      // Upsert Mongo profile
      const displayName = (await users.findOne({ email }))?.displayName || '';
      ...
      if (result.error) {
        ...
        await users.updateOne({ email }, { $set: { welcomeEmailSentAt: new Date(), mustChangePassword: true } });
      }
    } catch (err) {
