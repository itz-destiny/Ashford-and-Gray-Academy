...
const EMAIL = 'Schistella46@gmail.com';
...
// Convert local Nigerian number starting with 0 to E.164 format for Firebase
const E164_PHONE = PHONE.startsWith('0') ? `+234${PHONE.slice(1)}` : PHONE;
  ...
  try {
    ...
    try {
      ...
      if (err.code === 'auth/user-not-found' || err.code === 'auth/user-not-found') {
        fbUser = await admin.auth().createUser({ email: EMAIL, password: tempPwd, emailVerified: true, displayName: DISPLAY_NAME, phoneNumber: E164_PHONE });
