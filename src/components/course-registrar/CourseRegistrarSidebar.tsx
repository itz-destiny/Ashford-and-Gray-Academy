...
const navItems = [
    { href: "/course-registrar", label: "Study Plan Home", icon: LayoutDashboard },
    { href: "/course-registrar/courses", label: "Academy Programs", icon: BookOpen },
    ...
    return (
        <aside className={cn(
            ...
            <nav className="flex-1 px-8 space-y-2 overflow-y-auto custom-scrollbar">
                <p className="px-5 text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-6">Program Management</p>
                ...
                <div className="pt-10">
                    ...
                    <Link
                        ...
                        className={cn(
                            "group flex items-center gap-4 px-6 py-4 rounded-[2rem] transition-all duration-500",
                            pathname === "/course-registrar/settings" ? "bg-[#0B1F3A] text-white" : "text-slate-400 hover:bg-slate-50 hover:text-[#0B1F3A]"
                        )}
                    >
                        <Settings className={cn("w-5 h-5", pathname === "/course-registrar/settings" ? "text-[#C8A96A]" : "text-slate-300 group-hover:text-[#0B1F3A]")} />
            ...
            <div className="p-8 mt-auto border-t border-slate-50">
                <div className="flex items-center gap-4 px-4 py-4 bg-slate-50 rounded-[2.5rem] border border-slate-100 mb-6">
                    ...
                    <div className="overflow-hidden">
                        <p className="text-sm font-black text-[#0B1F3A] truncate">{user?.displayName || "Program Manager"}</p>
                        <p className="text-[10px] font-black text-[#C8A96A] uppercase tracking-wider mt-0.5">Academy Programs</p>
                    </div>
                </div>
                <button
                    onClick={async () => {
