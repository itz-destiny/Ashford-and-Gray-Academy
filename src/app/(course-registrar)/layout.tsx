...
import { courseRegistrarTourSteps } from "@/lib/tutorial-steps/course-registrar";

export default function CourseRegistrarLayout({ children }: { children: React.ReactNode }) {
    return (
        <RoleGuard allowed={["course_registrar"]}>
            <PortalTutorial steps={courseRegistrarTourSteps} />
                    ...
                    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 bg-[#FAF9F6]/90 backdrop-blur-md px-6 md:px-10 border-t-4 border-t-[#C8A96A] border-b border-b-[#0B1F3A]/5">
                        ...
                        <div className="flex items-center gap-3 ml-auto lg:ml-0">
                            <Button variant="ghost" size="icon" className="text-slate-400 hover:text-[#0B1F3A] hover:bg-[#0B1F3A]/5 rounded-none" asChild>
                                <Link href="/course-registrar/settings"><Settings className="h-4 w-4" /></Link>
                            </Button>
