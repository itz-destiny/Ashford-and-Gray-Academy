      ...
      <main>
                ...
                <Card key={item.id} className="overflow-hidden group">
                  <div className="relative">
                    ...
                    <div className="absolute top-2 left-2 flex items-center gap-2">
                       <Badge variant="secondary" className="text-xs uppercase">{item.type}</Badge>
                       {item.rating && (
                       ...
                       {item.date && item.type !== 'COURSE' && <Badge variant="secondary">{item.date}</Badge>}
                    </div>
                  </div>
                  <CardContent className="p-4">
                    <h3 className="font-semibold truncate group-hover:text-primary">{item.title}</h3>
                    <p className="text-sm text-muted-foreground mt-1">{item.author || item.description}</p>
                    <div className="flex justify-between items-center mt-4">
                      ...
                      <Button variant={item.action === 'Register' ? 'primary' : 'outline'} size="sm">
                        {item.action || 'View'}
                      </Button>
                    </div>
                  </CardContent>
        ...
        <section className="py-24 bg-secondary">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8">
              {testimonials.map(t => (
                <Card key={t.id} className="p-6 text-left bg-background">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
            ...
            <div>
              ...
              <ul className="mt-4 space-y-2">
                ...
                <li><Link href="#" className="text-muted-foreground hovertext-primary">Community</Link></li>
