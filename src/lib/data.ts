
import type { Course, AppEvent } from './types';

// This data is now used for seeding the database, not for direct display.
export const coursesToSeed: Omit<Course, 'id' | 'progress'>[] = [
  {
    title: 'Protocol and Crown Control Training Etiquette',
    description: 'Master the art of etiquette for formal and professional settings.',
    category: 'Certification',
    instructor: { name: 'Dr. Emily Post', avatarUrl: 'https://picsum.photos/seed/ins1/100/100', verified: true },
    rating: 4.9,
    reviews: 1530,
    duration: 4,
    level: 'All Levels',
    price: 350000,
    imageUrl: '/course1.jpeg',
    imageHint: 'formal dining setup',
  },
  {
    title: 'Cleaning and Maintenance Fundamentals',
    description: 'Learn industry standards for commercial and residential cleaning.',
    category: 'Certification',
    instructor: { name: 'Mr. Don Aslett', avatarUrl: 'https://picsum.photos/seed/ins2/100/100', verified: false },
    rating: 4.7,
    reviews: 890,
    duration: 6,
    level: 'Beginner',
    price: 350000,
    imageUrl: '/course2.jpeg',
    imageHint: 'cleaning supplies',
  },
  {
    title: 'Book Keeping and Inventory Management',
    category: 'Certification',
    description: 'Essential skills for managing finances and stock for small businesses.',
    instructor: { name: 'Ms. Rachel Cruz', avatarUrl: 'https://picsum.photos/seed/ins3/100/100', verified: true },
    rating: 4.8,
    reviews: 2100,
    duration: 8,
    level: 'Intermediate',
    price: 350000,
    imageUrl: '/course3.jpeg',
    imageHint: 'ledger and calculator',
  },
  {
    title: 'Fine Dining Etiquette & Service',
    category: 'Certification',
    description: 'Advanced techniques for fine dining service and guest experience.',
    instructor: { name: 'Mr. Will Guidara', avatarUrl: 'https://picsum.photos/seed/ins4/100/100', verified: true },
    rating: 4.9,
    reviews: 1800,
    duration: 5,
    level: 'Advanced',
    price: 350000,
    imageUrl: '/course4.jpeg',
    imageHint: 'fine dining restaurant',
  },
  {
    title: 'Professional House-Keeping',
    category: 'Certification',
    description: 'Comprehensive training for professional housekeeping in the hotel industry.',
    instructor: { name: 'Ms. Marie Kondo', avatarUrl: 'https://picsum.photos/seed/ins5/100/100', verified: false },
    rating: 4.8,
    reviews: 1250,
    duration: 10,
    level: 'Intermediate',
    price: 350000,
    imageUrl: '/course5.jpeg',
    imageHint: 'hotel room cleaning',
  },
  {
    title: 'Advanced Cleaning and Maintenance',
    category: 'Certification',
    description: 'Specialized techniques for floor care, restoration, and sanitation.',
    instructor: { name: 'Mr. Don Aslett', avatarUrl: 'https://picsum.photos/seed/ins6/100/100', verified: false },
    rating: 4.6,
    reviews: 750,
    duration: 12,
    level: 'Advanced',
    price: 350000,
    imageUrl: '/course6.jpeg',
    imageHint: 'industrial cleaning machine',
  },
  {
    title: 'Customer Service Excellence',
    category: 'Certification',
    description: 'Develop key skills for providing outstanding customer service.',
    instructor: { name: 'Mr. Shep Hyken', avatarUrl: 'https://picsum.photos/seed/ins7/100/100', verified: true },
    rating: 4.8,
    reviews: 2500,
    duration: 4,
    level: 'Beginner',
    price: 350000,
    imageUrl: '/course7.jpeg',
    imageHint: 'customer service representative',
  },
  {
    title: 'Housekeeping and Domestic Management Certificate',
    description: 'A comprehensive certification covering all aspects of professional housekeeping and domestic management.',
    curriculum: [
      'Introduction to Housekeeping',
      'Cleaning Techniques & Sanitization',
      'Bed Making & Room Setup',
      'Deep Cleaning & Maintenance',
      'Inventory & Storage Management',
      'Hygiene Protocols',
      'Managing Household Staff',
      'Understanding Client Preferences',
      'Emergency Response',
      'Final Assessment',
      'Introduction to Wardrobe Mgmt',
      'Fabric Care & Cleaning Techniques',
      'Luxury Garment Handling',
      'Ironing, Pressing & Folding',
      'Stain Removal & Dry Cleaning',
      'Organizing Closets',
      'Shoe & Leather Care',
      'Travel Wardrobe Management',
      'Budgeting & Inventory Mgmt',
      'Final Practical'
    ],
    category: 'Certification',
    instructor: { name: 'Sarah Ashford', avatarUrl: 'https://picsum.photos/seed/sarah/100/100', verified: true },
    rating: 4.8,
    reviews: 120,
    duration: 12,
    level: 'All Levels',
    price: 350000,
    imageUrl: '/housekeeping_certificate.png',
    imageHint: 'professional housekeeping setup',
  },
  {
    title: 'Hospitality Management Certificate',
    description: 'Master the skills required for high-end hospitality management, from butler services to guest relations.',
    curriculum: [
      'Role of a Butler & Concierge',
      'Guest Services & Protocol',
      'Table Setting & Fine Dining',
      'Managing High-End Clients',
      'Confidentiality & Discretion',
      'Advanced Communication',
      'VIP Event Coordination',
      'Travel & Lifestyle Management',
      'Conflict Resolution',
      'Final Practical',
      'Luxury Hospitality Foundations',
      'Cultural Sensitivity',
      'Personalised Guest Experience',
      'Communication Skills',
      'Handling Complaints',
      'Customer Retention Strategies',
      'Upselling & Cross-Selling',
      'Case Studies in Guest Relations',
      'Crisis Management',
      'Certification & Assessment'
    ],
    category: 'Certification',
    instructor: { name: 'James Gray', avatarUrl: 'https://picsum.photos/seed/james/100/100', verified: true },
    rating: 4.9,
    reviews: 85,
    duration: 14,
    level: 'Advanced',
    price: 350000,
    imageUrl: '/hospitality_management_certificate.png',
    imageHint: 'luxury hospitality service',
  },
  {
    title: 'Events and Protocol Management Certificate',
    description: 'Expert training in planning and executing high-profile events with strict adherence to protocol.',
    curriculum: [
      'Introduction to Event Planning',
      'Venue Selection & Event Design',
      'Vendor Coordination & Contracts',
      'Budgeting & Finance',
      'VIP & Celebrity Event Management',
      'Security & Risk Management',
      'Crisis Communication',
      'Marketing & Promotion',
      'Event Execution & Coordination',
      'Post-Event Evaluation',
      'Intro to Protocol & Etiquette',
      'Cultural Sensitivities',
      'High-Profile Guest Management',
      'Crowy Psychology',
      'Security & Emergency Protocols',
      'Official Documentation',
      'Crisis Handling',
      'International Protocol',
      'Case Studies in Successful Protocol',
      'Final Practical'
    ],
    category: 'Certification',
    instructor: { name: 'Eleanor Vance', avatarUrl: 'https://picsum.photos/seed/eleanor/100/100', verified: true },
    rating: 4.7,
    reviews: 64,
    duration: 12,
    level: 'Intermediate',
    price: 350000,
    imageUrl: '/course1.jpeg',
    imageHint: 'sophisticated event setup',
  },
  {
    title: 'Executive Assistant Management Certificate',
    description: 'The definitive guide to becoming a top-tier executive assistant for high-net-worth individuals.',
    curriculum: [
      'Understanding VIP Concierge',
      'Luxury Lifestyle Management',
      'Personal Assistant Duties',
      'Exclusive Travel & Reservations',
      'Personal Styling & Image Mgmt',
      'Managing High-Net-Worth Clients',
      'Special Event Coordination',
      'Confidentiality in VIP Services',
      'Crisis Management in VIP Services',
      'Final Practical',
      'Introduction to Executive Assistance',
      'Professional Correspondence',
      'Scheduling & Time Mgmt',
      'Business Travel Planning',
      'Managing Corporate Events',
      'Leadership Support & Decision-Making',
      'Data & File Management',
      'Presentation & Public Speaking',
      'Conflict Resolution',
      'Final Project'
    ],
    category: 'Certification',
    instructor: { name: 'Robert Sinclair', avatarUrl: 'https://picsum.photos/seed/robert/100/100', verified: true },
    rating: 4.8,
    reviews: 92,
    duration: 10,
    level: 'All Levels',
    price: 350000,
    imageUrl: '/course3.jpeg',
    imageHint: 'professional executive office',
  },
  {
    title: 'Hospitality and Global Relationship Certificate',
    description: 'Navigate the complex world of global business and diplomacy within the hospitality sector.',
    curriculum: [
      'Introduction to Global Business',
      'Trade Policies & Agreements',
      'Diplomatic Protocols & Etiquette',
      'International Negotiation Strategies',
      'Cross-Cultural Communication',
      'Case Studies in Diplomacy',
      'Global Business Ethics',
      'International Crisis Management',
      'Career Opportunities in Diplomacy',
      'Final Project',
      'Fundamentals of Negotiation',
      'Mediation Strategies',
      'Conflict Resolution Framework',
      'Handling Difficult People',
      'Workplace Dispute Management',
      'Cross-Cultural Negotiation',
      'Business Contract Negotiation',
      'Case Studies in Conflict Resolution',
      'Final Negotiation Simulation',
      'Assessment & Certification'
    ],
    category: 'Certification',
    instructor: { name: 'Ambassador Linda Thomas', avatarUrl: 'https://picsum.photos/seed/linda/100/100', verified: true },
    rating: 5.0,
    reviews: 45,
    duration: 16,
    level: 'Advanced',
    price: 350000,
    imageUrl: '/course4.jpeg',
    imageHint: 'international diplomatic summit',
  },
  {
    title: 'Diploma in Hospitality Management',
    description: 'Our most comprehensive hospitality program, combining housekeeping, domestic management, butler services, and high-end guest relations into a single prestigious qualification.',
    curriculum: [
      'Introduction to Housekeeping', 'Cleaning Techniques & Sanitization', 'Bed Making & Room Setup', 'Deep Cleaning & Maintenance',
      'Inventory & Storage Management', 'Hygiene Protocols', 'Managing Household Staff', 'Understanding Client Preferences',
      'Emergency Response', 'Final Assessment', 'Role of a Butler & Concierge', 'Guest Services & Protocol',
      'Table Setting & Fine Dining', 'Managing High-End Clients', 'Confidentiality & Discretion', 'Advanced Communication',
      'VIP Event Coordination', 'Travel & Lifestyle Management', 'Conflict Resolution', 'Final Practical',
      'Luxury Hospitality Foundations', 'Cultural Sensitivity', 'Personalised Guest Experience', 'Communication Skills',
      'Handling Complaints', 'Customer Retention Strategies', 'Upselling & Cross-Selling', 'Case Studies in Guest Relations',
      'Crisis Management', 'Certification & Assessment', 'Introduction to Wardrobe Mgmt', 'Fabric Care & Cleaning Techniques',
      'Luxury Garment Handling', 'Ironing, Pressing & Folding', 'Stain Removal & Dry Cleaning', 'Organizing Closets',
      'Shoe & Leather Care', 'Travel Wardrobe Management', 'Budgeting & Inventory Mgmt', 'Final Practical'
    ],
    category: 'Diploma',
    instructor: { name: 'James Gray', avatarUrl: 'https://picsum.photos/seed/james/100/100', verified: true },
    rating: 4.9,
    reviews: 210,
    duration: 36,
    level: 'Advanced',
    price: 1500000,
    imageUrl: '/hospitality_management_certificate.png',
    imageHint: 'comprehensive hospitality training',
  },
  {
    title: 'Diploma in Corporate Concierge, Innovation and Leadership',
    description: 'Exclusively designed for high-level personal assistants and lifestyle managers seeking to master corporate concierge services and modern leadership.',
    curriculum: [
      'Foundations of Corporate Concierge', 'Professional Ethics in Concierge', 'Roles & Responsibilities', 'Client Management & Etiquette',
      'Cultural Awareness & Corporate Etiquette', 'Communication Skills Workshop', 'Lifestyle & Travel Management', 'Exclusive Travel Planning',
      'Personal Shopping & Styling', 'Simulation: VIP Travel Planning', 'Event & Vendor Coordination', 'Special Event Planning',
      'Vendor/Partner Management', 'Workshop: Concierge Event Brief', 'Confidentiality & Risk Management', 'Data Security in VIP Services',
      'Crisis Handling in VIP Services', 'Scenario Drill: Confidential Crisis', 'Technology & Innovation in Concierge', 'Concierge Desk Operations',
      'Workflow Management', 'Apps, AI & automation', 'Tech Tools Demo', 'Leadership in Concierge Services',
      'Emotional Intelligence for Managers', 'Motivations Teams', 'Designing a Service Blueprint', 'Concierge Service Excellence',
      'Luxury Service Standards', 'Role-play: Client Interaction'
    ],
    category: 'Diploma',
    instructor: { name: 'Robert Sinclair', avatarUrl: 'https://picsum.photos/seed/robert/100/100', verified: true },
    rating: 4.8,
    reviews: 155,
    duration: 24,
    level: 'Advanced',
    price: 1500000,
    imageUrl: '/course3.jpeg',
    imageHint: 'executive leadership and concierge',
  },
  {
    title: 'Diploma in Event and Protocol Management',
    description: 'A master-level program for event professionals, combining advanced event design with international protocol and VIP management.',
    curriculum: [
      'Introduction to Event Planning', 'Venue Selection & Event Design', 'Vendor Coordination & Contracts', 'Budgeting & Finance',
      'VIP & Celebrity Event Management', 'Security & Risk Management', 'Crisis Communication', 'Marketing & Promotion',
      'Event Execution & Coordination', 'Post-Event Evaluation', 'Intro to Protocol & Etiquette', 'Cultural Sensitivities',
      'High-Profile Guest Management', 'Crowd Psychology', 'Security & Emergency Protocols', 'Official Documentation',
      'Crisis Handling', 'International Protocol', 'Case Studies in Successful Protocol', 'Final Practical',
      'Understanding VIP Concierge', 'Luxury Lifestyle Management', 'Personal Assistant Duties', 'Exclusive Travel & Reservations',
      'Personal Styling & Image Mgmt', 'Managing High-Net-Worth Clients', 'Special Event Coordination', 'Confidentiality in VIP Services',
      'Crisis Management in VIP Services', 'Final Practical', 'Introduction to Executive Assistance', 'Professional Correspondence',
      'Scheduling & Time Mgmt', 'Business Travel Planning', 'Managing Corporate Events', 'Leadership Support & Decision-Making',
      'Data & File Management', 'Presentation & Public Speaking', 'Conflict Resolution', 'Final Project'
    ],
    category: 'Diploma',
    instructor: { name: 'Eleanor Vance', avatarUrl: 'https://picsum.photos/seed/eleanor/100/100', verified: true },
    rating: 4.9,
    reviews: 180,
    duration: 32,
    level: 'Advanced',
    price: 1500000,
    imageUrl: '/course1.jpeg',
    imageHint: 'advanced event and protocol setup',
  },
  {
    title: 'Diploma in Professional Development & Global Relations',
    description: 'Master the art of diplomatic relations, international negotiation, and high-level leadership in a globalized world.',
    curriculum: [
      'Leadership Theories & Styles', 'Emotional Intelligence', 'Team Leadership', 'Conflict Resolution',
      'Decision Making', 'Time Management', 'Corporate Culture', 'Leadership Case Studies',
      'Ethical Leadership', 'Leadership Simulation', 'Introduction to Global Business', 'Trade Policies & Agreements',
      'Diplomatic Protocols & Etiquette', 'International Negotiation Strategies', 'Cross-Cultural Communication', 'Case Studies in Diplomacy',
      'Global Business Ethics', 'International Crisis Management', 'Career Opportunities in Diplomacy', 'Final Project',
      'Career Planning & Goal Setting', 'Workplace Communication', 'Resume & Cover Letter Writing', 'Professional Networking',
      'Personal Branding', 'Job Interview Techniques', 'Office Politics & Conflict', 'Ethics & Professional Conduct',
      'Work-Life Balance Strategies', 'Final Career Plan Presentation', 'Fundamentals of Negotiation', 'Mediation Strategies',
      'Conflict Resolution Framework', 'Handling Difficult People', 'Workplace Dispute Management', 'Cross-Cultural Negotiation',
      'Business Contract Negotiation', 'Case Studies in Conflict Resolution', 'Final Negotiation Simulation', 'Assessment & Certification'
    ],
    category: 'Diploma',
    instructor: { name: 'Ambassador Linda Thomas', avatarUrl: 'https://picsum.photos/seed/linda/100/100', verified: true },
    rating: 5.0,
    reviews: 120,
    duration: 40,
    level: 'Advanced',
    price: 1500000,
    imageUrl: '/course4.jpeg',
    imageHint: 'global relations and diplomacy',
  },
  {
    title: 'Diploma in Business Innovation & Entrepreneurship',
    description: 'The ultimate entrepreneurial toolkit, covering business administration, strategic finance, and digital branding for the modern innovator.',
    curriculum: [
      'Fundamentals of Business Administration', 'Strategic Planning', 'Organisational Structures', 'Business Ethics',
      'Financial Management', 'Competitive Analysis', 'Business Negotiation', 'Case Studies',
      'Risk Management', 'Final Project', 'Intro to Entrepreneurship', 'Identifying Business Opportunities',
      'Business Model Development', 'Funding Options', 'Digital Entrepreneurship', 'Legal Considerations',
      'Pitching to Investors', 'Scaling a Business', 'Case Studies', 'Final Pitch',
      'Introduction to Economics', 'Micro vs. Macroeconomics', 'Financial Markets & Investments', 'Business Finance',
      'Financial Planning', 'Economic Policies', 'Cryptocurrency Trends', 'Case Studies',
      'Risk Management', 'Assessment', 'Basics of Corporate Communication', 'Brand Development Strategies',
      'Social Media & Digital Branding', 'PR & Crisis Management', 'Persuasive Business Writing', 'Visual Identity & Design',
      'Brand Case Studies', 'Internal & External Communication', 'Global Branding', 'Final Project'
    ],
    category: 'Diploma',
    instructor: { name: 'Robert Sinclair', avatarUrl: 'https://picsum.photos/seed/robert/100/100', verified: true },
    rating: 4.7,
    reviews: 140,
    duration: 36,
    level: 'Advanced',
    price: 1500000,
    imageUrl: '/course7.jpeg',
    imageHint: 'innovation and business strategy',
  },
  {
    title: 'Executive MBA in Hospitality and Tourism Management',
    description: 'The pinnacle of professional achievement at Ashford & Gray. This elite program is designed for senior leaders and entrepreneurs seeking to master the global landscape of hospitality and business innovation.',
    curriculum: [
      'HTM 500: Global Business Environment and Strategy',
      'HTM 501: Managerial Economics & Decision Making',
      'HTM 502: Financial Accounting & Analysis',
      'HTM 503: Organizational Behavior & Leadership',
      'HTM 504: Corporate Finance & Investment Decisions',
      'HTM 505: Marketing Strategy & Digital Transformation',
      'HTM 506: Operations & Supply Chain Management',
      'HTM 507: Business Innovation & Entrepreneurship',
      'HTM 508: Corporate Governance, Ethics & Legal Environment',
      'HTM 509: Strategic Human Resource Management',
      'HTM 510: Data Analytics and Business Intelligence',
      'Specialization: Hospitality & Tourism Leadership',
      'Specialization: Event Management & Protocol',
      'Specialization: Corporate Communications & Brand Strategy',
      'Specialization: Financial Intelligence & Wealth Management',
      'Specialization: Global Entrepreneurship & Innovation',
      'Research Project & Thesis Execution'
    ],
    category: 'Executive MBA',
    instructor: { name: 'Dr. Alistair Ashford', avatarUrl: 'https://picsum.photos/seed/alistair/100/100', verified: true },
    rating: 5.0,
    reviews: 28,
    duration: 104,
    level: 'Advanced',
    price: 2500000,
    imageUrl: '/course4.jpeg',
    imageHint: 'executive boardroom meeting',
  },
];


export const eventsToSeed: Omit<AppEvent, 'id'>[] = [
  {
    title: 'Protocol and Crown Control Training Etiquette',
    category: 'Workshop',
    date: '2024-11-15',
    location: 'Virtual',
    price: 150,
    imageUrl: '/course1.jpeg',
    imageHint: 'formal dining setup',
    organizer: 'Etiquette Experts'
  },
  {
    title: 'Cleaning and Maintenance Fundamentals',
    category: 'Seminar',
    date: '2024-12-02',
    location: 'New York, NY',
    price: 99,
    imageUrl: '/course2.jpeg',
    imageHint: 'cleaning supplies',
    organizer: 'CleanCo'
  },
  {
    title: 'Book Keeping and Inventory Management',
    category: 'Webinar',
    date: '2024-11-20',
    location: 'Online',
    price: 220,
    imageUrl: '/course3.jpeg',
    imageHint: 'ledger and calculator',
    organizer: 'Biz Solutions'
  },
  {
    title: 'Fine Dining Etiquette & Service',
    category: 'Workshop',
    date: '2024-12-10',
    location: 'San Francisco, CA',
    price: 350,
    imageUrl: '/course4.jpeg',
    imageHint: 'fine dining restaurant',
    organizer: 'Hospitality Masters'
  },
];

export const mockLiveClassTranscript = `Instructor: Good morning, everyone. Welcome to Introduction to Quantum Physics. Today, we're going to start with the absolute basics, the foundational concepts that everything else is built upon. So, the first and most mind-bending concept is wave-particle duality.

Student 1: Uh, professor? What does that even mean?

Instructor: Great question! It means that tiny things, like electrons or photons, don't just behave like tiny billiard balls. They also behave like waves, spreading out in space. It's weird, I know. An electron can be in multiple places at once until you measure it.

Student 2: So, when we're not looking, it's a wave? And when we look, it's a particle?

Instructor: Exactly! The act of observation or measurement 'collapses' the wave function into a single, definite state. This is called the observer effect. Think of it like this: before you open the box, Schrödinger's cat is both alive and dead. The moment you look, it's one or the other. We'll be doing a lab on the double-slit experiment next week which demonstrates this perfectly.

Student 1: So this applies to everything? Like, is this desk also a wave?

Instructor: Technically, yes! Everything has a wavelength, it's called the de Broglie wavelength. But for large objects, the wavelength is so incredibly tiny that it's completely undetectable. You don't have to worry about walking through walls, even if your particles are waves. The probability is practically zero.

Student 3: You mentioned "quantized" states. What does that mean?

Instructor: Another excellent question. It means that certain properties, like the energy of an electron in an atom, can only have specific, discrete values. It can't be just any value. It's like a staircase, not a ramp. An electron can be on step one or step two, but never in between. This is a fundamental departure from classical physics.

Instructor: Alright, that's a lot to take in. The key takeaways for today are: wave-particle duality, the observer effect, and quantized energy levels. These are the pillars of quantum mechanics. Read Chapter 1 for a deeper dive. Any final questions?

Student 2: My brain hurts.

Instructor: (Chuckles) That's a very common side effect. It means you're starting to get it. Class dismissed.`;

export const mockUser = {
  name: 'Alex Johnson',
  email: 'alex.j@example.com',
  avatarUrl: 'https://picsum.photos/seed/user/100/100',
  role: 'student' as 'student' | 'admin' | 'instructor',
};

export const mockResources = [
  { id: '1', title: 'Quantum Mechanics 101', type: 'PDF', course: 'Physics 101', dateAdded: '2023-10-15' },
  { id: '2', title: 'Intro to UX Design', type: 'Video', course: 'UX Design', dateAdded: '2023-10-12' },
  { id: '3', title: 'Data Structures Cheatsheet', type: 'PDF', course: 'Data Science', dateAdded: '2023-10-10' },
  { id: '4', title: 'Bauhaus Movement', type: 'Slides', course: 'Design History', dateAdded: '2023-10-08' },
];

export const mockEvents = [
  { id: '1', title: 'Physics 101 Live', course: 'Quantum Physics', startTime: '10:00 AM', type: 'Live Class' },
  { id: '2', title: 'Python Quiz 2 Due', course: 'Data Science', startTime: '11:59 PM', type: 'Quiz Due' },
  { id: '3', title: 'Bauhaus Essay Due', course: 'Design History', startTime: '11:59 PM', type: 'Assignment' },
]

export const mockCourses = [
  ...coursesToSeed
]

export const mockAssignments = [
  { id: '1', course: 'Design History', title: 'Essay: Evolution of Bauhaus', dueDate: 'Tomorrow' },
  { id: '2', course: 'Data Science', title: 'Python Quiz 2', dueDate: 'In 3 days' },
];


export const mockRecentEnrollments = [
  { id: '1', student: { name: 'Alice', avatarUrl: 'https://picsum.photos/seed/s1/100' }, course: 'Biology 101', date: '2023-10-20', status: 'Approved' },
  { id: '2', student: { name: 'Bob', avatarUrl: 'https://picsum.photos/seed/s2/100' }, course: 'Chemistry 202', date: '2023-10-19', status: 'Approved' },
  { id: '3', student: { name: 'Charlie', avatarUrl: 'https://picsum.photos/seed/s3/100' }, course: 'Physics 101', date: '2023-10-22', status: 'Pending' },
];

export const mockInstructorMessages: any[] = [
  { id: '1', student: { name: 'Maria Garcia', avatarUrl: 'https://picsum.photos/seed/s4/100' }, message: 'I have a question about the final project deadline.', time: '2h ago' },
  { id: '2', student: { name: 'David Lee', avatarUrl: 'https://picsum.photos/seed/s5/100' }, message: 'Could you clarify the lecture notes from yesterday?', time: '5h ago' },
  { id: '3', student: { name: 'Sophia Chen', avatarUrl: 'https://picsum.photos/seed/s6/100' }, message: 'Thank you for the extension!', time: '1d ago' },
];

